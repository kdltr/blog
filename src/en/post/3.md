...
After doing so, it will translate all use of imported symbols to their right symbol in the module
...
If it will print these kind of lines if it still depends on shared libraries:
...
One other I wanted is taking advantage of hardware acceleration offered by GPUs.
This is why I went on to the second big change if the code base: using the [SDL Renderer API][sdl-render].
